...
class Trainer():
    def __init__(self, model, optimizer, cont_capacity=None,
        ...
        np.random.seed(1265)
        pz_mu = np.random.normal(0, 1, (1, 10))
        self.pz_mu = torch.tensor(pz_mu).to(device='cuda:0')
    ...
    def _loss_function(self, data, recon_data, latent_dist):
        ...
        if self.model.is_continuous:
            ...
            kl_cont_loss = self._cs_loss(mean, logvar)
    ...
    def _cs_loss(self, mu, logvar):
        std = torch.exp(logvar)
        pz_std = torch.ones(1, 10).to(device='cuda:0')
        res1 = self.compute_CS(mu, std, self.pz_mu, pz_std)
        res2 = self.compute_CS(mu, std, mu, std)
        res3 = self.compute_CS(self.pz_mu, pz_std, self.pz_mu, pz_std)

        c1 = torch.sum(res1) / self.batch_size
        c2 = torch.sum(res2) / self.batch_size
        c3 = torch.sum(res3)
        # cont_loss
        csd_loss = torch.sum(-torch.log(0.1 * c1) +
                             0.5 * torch.log(c2) +
                             0.5 * torch.log(0.1 * 0.1 * c3))
        return csd_loss
    ...
    def compute_CS(self, m_mu, m_std, pz_mu, pz_std):
        res = []
        for i in range(10):
            for j in range(10):
                res.append((1/torch.sqrt(torch.tensor(2*self.pi*(m_std[:, i] ** 2+pz_std[:, j] ** 2)))) * torch.exp(torch.tensor(-((m_mu[:, i]-pz_mu[:, i]) ** 2)/(2*(m_std[:, i] ** 2+pz_std[:, j] ** 2)))))
        return torch.cat(tuple(res), 0)